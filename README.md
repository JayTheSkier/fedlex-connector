...
The server supports two transports: **stdio** for local use (Claude Code, Claude Desktop) and **HTTP** for remote use (claude.ai, ChatGPT-compatible MCP clients). You can use one or both.
...
ChatGPT-compatible MCP endpoint:
...
For ChatGPT Desktop or ChatGPT Developer Mode, point the MCP app registration at `https://your-domain/mcp`. If you deploy under a new hostname, set `ALLOWED_HOSTS` to a comma-separated list of accepted hosts:

```bash
PORT=3000 ALLOWED_HOSTS=mcp.example.com,localhost,127.0.0.1 node build/index.js
```

See [docs/chatgpt-compatibility.md](docs/chatgpt-compatibility.md) for the plugin development workflow.
...
ChatGPT-compatible retrieval search. Searches currently in-force Swiss federal legislation titles across German, French, and Italian, then returns citable Fedlex result IDs for `fetch`.
...
ChatGPT-compatible retrieval fetch. Fetches text for a result returned by `search`. Large acts are returned page by page with a `next_id` in the metadata.

| Parameter | Type   | Required | Description                    |
|-----------|--------|----------|--------------------------------|
| `id`      | string | Yes      | ID returned by the search tool |
...
docs/            # Development notes, agile plan, ChatGPT compatibility workflow
plugins/         # Repo-local plugin package for Codex/ChatGPT development
test/            # Network-free compatibility tests
...
| `npm test`           | Build and run network-free tests         |
